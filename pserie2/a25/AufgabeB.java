...
public class AufgabeB implements PlugIn {
	...
	public void run(String arg) {
		...
		// Calculate AND image
		ImagePlus andIp = NewImage.createImage("AND", ip1.getWidth(),
				ip1.getHeight(), 1, ip1.getBitDepth(), NewImage.FILL_BLACK);
		ImageProcessor andProcessor = andIp.getProcessor();
		for (int i = 0; i < andProcessor.getWidth(); i++) {
			for (int j = 0; j < andProcessor.getHeight(); j++) {
				andProcessor.set(i, j,
						Math.min(processor1.get(i, j), processor2.get(i, j)));
			}
		}
